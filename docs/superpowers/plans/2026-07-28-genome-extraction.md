...
> **STATUS: COMPLETE.** All 14 tasks are implemented and committed (`0a205e7`…`12a747b`). The unticked `- [ ]` checkboxes below are a bookkeeping lapse, not evidence of incomplete work — verified by audit on 2026-07-28: every promised export exists, every promised test file exists, `npm test` is 75 files / 738 tests green, `npm run build` is clean.
>
> **Two naming drifts post-date this document. `docs/reference/genome.md` is the live contract; prefer it over this plan where they disagree.**
> - The cost-model gene was renamed `tokenWeights` → **`weightedTokenSpend`** in `12a747b`. This plan still says `tokenWeights` in 34 places; the schema, the code and the reference doc all say `weightedTokenSpend`.
> - Task 11's prose says the correlator runs inside `ost_status`. It does not, and must not — see the Fail-open telemetry constraint below. Its caller is the Phase 3 harness.
...
- **Fail-open telemetry.** `recordUsageEvent` and `recordAttention` never throw, by contract. The correlator inherits this: a missing transcript dir, an unreadable session file, or a malformed `usage` object degrades to an empty result, never a throw. (The correlator's caller is the Phase 3 harness, *not* `ost_status` — `eval/` read surfaces are write-free and `markCorrelated` writes a cursor; see Task 11. Fail-open still holds, because a correlator that threw would kill the run it was measuring and a dead harness must not read as a bad genome.)
