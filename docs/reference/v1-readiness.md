...
> box where a full run earlier the same session took **343 s**. Run alone the two take
...
> `docs/reference/workflow-grammar.md` is now generated by `npm run gen:skill` beside the
> skeleton, and the generator refuses to publish a page whose claims about the parser the
> parser contradicts. Two things the build found that the node did not say. **The surface in
> the node's title cannot be made to publish anything** — the `Workflow` tool's grammar is not
> ours to emit, so what ships is a reconstruction a consumer maintains, and the drift check
> only runs in one direction: a construct this repository claims is refused is checked against
> the parser, while one the surface quietly began accepting stays invisible until a refusal
> records it. The page states that limit in its own body rather than reading as the surface's
> word. And the enumeration the assumption test called small is not one list but two — six of
> the eleven rejected constructs *parse* and throw at run time, so no parse check sees them
> and the only other way to learn them is a submission that starts, spends tokens and dies.
> The file added before that is
