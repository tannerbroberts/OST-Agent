...
- Record the Outcome as achieved, in a status or in prose. Whether the mandate the whole tree serves was MET is decided by a real-world signal the operator declared in `ost.config.yaml` — a file no tool here can write — plus a person reading it on the CLI (`ost-agent outcome-signal --verdict met --by "<who>" --reading "<the number>"`), which files it under the reserved heading '## Outcome Signal'. Every write that would land that verdict on the root is refused until then, and the refusal names what is missing. Say what progress you observed; leave the verdict alone. It is the one claim that, made about itself, clears every gate beneath it at once with nothing above it left to notice.
...
All are exposed by the `ost-agent` MCP server (installed as a plugin, the names a session mints are `mcp__plugin_ost-agent_ost-agent__ost_*`; registered directly, `mcp__ost-agent__ost_*`):
