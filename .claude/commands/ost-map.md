...
1. Call `ost_ingest_inbox` to capture any notes dropped since the last run, then call `ost_next_work`; take its `unmappedEvidence` list. If empty, say so and stop.
