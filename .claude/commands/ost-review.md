...
2. For each, present the decision compactly and let the human choose:
   - **promote** → `ost_set_status` to `validated` — but ONLY with human-provided evidence, which the human must state in the note. Never promote on your own judgment; if no evidence is given, leave it `unvalidated`.
