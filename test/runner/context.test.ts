...
describe("buildPassContext adapter wiring", () => {
  test("the drop folders and the mechanical usage trace by default", () => {
    ...
    expect(ctx.sources.map((s) => s.name).sort()).toEqual(["deposit", "friction", "inbox", "retrospective", "usage"]);
  });
  ...
  test("enabling the transcript adapter with an explicit path adds the source", () => {
    ...
    const ctx = buildPassContext(dir);
    expect(ctx.sources.map((s) => s.name).sort()).toEqual(["deposit", "friction", "inbox", "retrospective", "transcript", "usage"]);
  });
  ...
  test("every declared channel lands in exactly one of the two lists", () => {
    ...
    enableAtlassian();
    const ctx = buildPassContext(dir);
    expect(accountedFor(ctx)).toEqual(["actions", "atlassian", "deposit", "friction", "inbox", "retrospective", "slack", "transcript", "usage"]);
    ...
    expect(accountedFor(withCreds)).toEqual(["actions", "atlassian", "deposit", "friction", "inbox", "retrospective", "slack", "transcript", "usage"]);
  ...
  test("enabling Atlassian with credentials adds the source", () => {
    ...
    process.env.ATLASSIAN_API_TOKEN = "atlassian-api-token-fixture";
    const ctx = buildPassContext(dir);
    expect(ctx.sources.map((s) => s.name).sort()).toEqual(["atlassian", "deposit", "friction", "inbox", "retrospective", "usage"]);
  });
