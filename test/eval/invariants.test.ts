...
describe("checkInvariants", () => {
  ...
  test("flags an assumption not under any solution", () => {
    ...
    expect(checkInvariants(tree, OUT).some((v) => v.rule === "assumption-mapped")).toBe(true);
  });
  ...
  test("flags a node that is both unvalidated-tagged and status validated", () => {
    ...
    expect(checkInvariants(tree, OUT).some((v) => v.rule === "no-self-validation")).toBe(true);
  });
});
