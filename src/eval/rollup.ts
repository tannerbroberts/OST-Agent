...
export interface BucketRollup {
  ...
  tested: number;
  refuted: number;
  /** Distinct `source` values beneath this bucket — how many independent voices it rests on. */
...
function rollUpBucket(bucket: OstNode, index: Map<string, OstNode>): BucketRollup {
    ...
    for (const b of rollup.buckets) {
      ...
      lines.push(
        `    ${b.opportunities} opportunity, ${b.solutions} solution, ${b.tests} test` +
          ...
          `, ${b.corroborators} source(s), rests on ${b.weakestRung ?? "nothing declared"}`,
      );
